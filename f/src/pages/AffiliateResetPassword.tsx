...
const AffiliateResetPassword = () => {
  ...
  const onSubmit = async (data: ResetPasswordFormInputs) => {
    ...
    try {
      const response = await axios.put(`${API_BASE_URL}/affiliated/reset-password/${token}`, {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-[#DBC166] mb-4">Password Reset Successful</h2>
          <p className="text-gray-600 mb-6">Your password has been updated successfully.</p>
          <button
            onClick={() => navigate('/affiliated/login')}
            className="w-full bg-gradient-to-r from-[#DBC166] to-[#C8A13A] text-white py-3 rounded-lg font-semibold hover:from-[#C8A13A] hover:to-[#B8943A] transition-all duration-300"
          >
            Back to Login
          </button>
        </motion.div>
